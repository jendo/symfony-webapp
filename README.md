...
While this stack is similar to the classic WAMP/LAMP, it uses Nginx and MariaDB for improved performance and open-source flexibility.
...
- **Data Persistence**: MariaDB data is stored in the `data/` folder.
...
```bash
git clone https://github.com/jendo/dockerized-symfony-skeleton.git
cd dockerized-symfony-skeleton
```
...
> These commands are mapped in the `Makefile` and use Composer scripts as defined in `project/composer.json`.  
> You can adjust the tool configuration (e.g., `phpstan.neon`, `.phpcs.xml`, etc.) directly in the `project/` directory.
...
- View Docker logs for errors:
  ...
  docker-compose logs
  ```
- Ensure no other services are using ports 90 (Nginx) or 8080 (Adminer).
